...
void Connection::handleMessage(const std::string& msg){	
	...
	if(msg.rfind("login:", 0) == 0){
		...
		PlayerManager::getInstance().bindPlayer2Conn(id, shared_from_this()); 
    ...
    if (result.empty()) {
        ...
        std::string reply = "room_enter:" + std::to_string(room_id);
        auto room = RoomManager::getInstance().getRoom(room_id);
		if (room) {
		    room->broadcast("room_enter:" + std::to_string(room_id));
		}
