...
Reactor::Reactor() : running_(false) {
    ...
    std::cout << "[Reactor] initialized, epoll_fd=" << epoll_fd_ << std::endl;
}
...
void Reactor::addFd(int fd, uint32_t events, EventCallback cb){
    ...
    if (::epoll_ctl(epoll_fd_, EPOLL_CTL_ADD, fd, &ev) == -1) {
        std::cerr << "[Reactor] epoll_ctl ADD failed for fd=" << fd 
                  << ": " << strerror(errno) << std::endl;
        return;
    }
    ...
    std::cout << "[Reactor] added fd=" << fd << ", events=" << events << std::endl;
}
